...
void
cylinder(Triangulation<dim, spacedim> &triangulation, const bool symm = false)
{
  (void) triangulation;
  (void) symm;
...

{
  ...
  // Turek grid
  else if (mesh_parameters.type == Parameters::Mesh::Type::turek &&
           !mesh_parameters.simplex)
    {
      cylinder(triangulation, false);
    }
  else
    throw std::runtime_error(
